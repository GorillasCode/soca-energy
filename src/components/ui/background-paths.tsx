...
export function FloatingPaths({
  ...
  return (
    <div className="pointer-events-none absolute inset-0">
      <svg
        className={cn("size-full text-foreground", svgClassName)}
        viewBox="0 0 696 316"
        fill="none"
        aria-hidden
      >
...
/** Animated paths layer for sections (place inside a `relative overflow-hidden` container). */
export function BackgroundPathsBackdrop({
  className,
  pathSvgClassName,
}: {
  className?: string
  pathSvgClassName?: string
}) {
  return (
    <div
      className={cn(
        "pointer-events-none absolute inset-0 overflow-hidden opacity-[0.22] dark:opacity-[0.18]",
        className,
      )}
      aria-hidden
    >
      <FloatingPaths position={1} svgClassName={pathSvgClassName} />
      <FloatingPaths position={-1} svgClassName={pathSvgClassName} />
    </div>
  )
}
    ...
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-white dark:bg-neutral-950">
      ...
      <div className="relative z-10 container mx-auto px-4 text-center md:px-6">
        ...
        >
          ...
          <div
            className="group relative inline-block overflow-hidden rounded-2xl border border-black/10 bg-gradient-to-b from-black/10 to-white/10 p-px shadow-lg backdrop-blur-lg transition-shadow duration-300 hover:shadow-xl dark:border-white/10 dark:from-white/10 dark:to-black/10 dark:hover:shadow-neutral-800/50"
          >
            <Button
              variant="ghost"
